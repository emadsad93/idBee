...
%     title([num2str(k) ': ' name])
    ...
    % get features
    feat_vector=getrefinefeats(zoom(k),limg2);
    data(k,:)=feat_vector;
end
...
for k=1:length(pixl)
    ...
    img=imread(char(pix(k).name));
    img=imresize(img(:,:,1:3),.25);
    limg = load(char(pixl(k).name));
    limg2=CleanCellBoundaries(limg.limg);
    ...
    h=imshow(label2rgb(limg2));
    set(h,'alphadata',0.6)
    title([num2str(k) ': ' char(pixl(k).name)])
    
    % get features
    data(k,:)=getrefinefeats(zoom(k),limg2);
end
save('t.mat','data','id','genus','species','subspecies',...
    'gender','lrwing','zoom');
scaleoptions='nanzscore';% DataScaling, NormalMaxMagnitude, none
[data mu s]=scalefeatures(data,scaleoptions);
imputemethod='zeros';
data=impute(data,imputemethod);
featind=1:20;
save('r.mat','data','id','genus','species','subspecies',...
    'gender','lrwing','zoom','scaleoptions','mu','s','imputemethod',...
    'featind');
rmpath('.\funcs','.\funcs\classifyfuncs')
...
load('r')% training data, includes scaling info (mu\s), feature indices
mu = nanmean(data);
s = nanstd(data);
...
%    title([num2str(k) ': ' name])
    ...
    % get features
    [feat_vector parts_inds num_parts]=getrefinefeats(zoom,limg2);
    % scale features
